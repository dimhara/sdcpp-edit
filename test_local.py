...
def main():
    ...
    parser.add_argument("--img", type=str, required=True, help="Path to input image (e.g., input.png)")
    parser.add_argument("--args", type=str, required=True, help="SD arguments (exclude the binary name)")
    parser.add_argument("--out", type=str, default="test_result.png", help="Where to save the output image")
    ...
    job_input = {
        "cmd_args": args.args,
        "init_image": encode_file(args.img)
    }
    ...
    if result.get("status") == "success":
        ...
        print("\n[STDOUT from Binary]:")
        print(result.get("stdout")[:500] + "... (truncated)" if result.get("stdout") else "None")
